    ...
    public partial class AppContainer : System.Windows.Forms.Panel
    {
        ...
        public string AppFilename
        {
            ...
            set
            {
                ...
                {
                    MessageBox.Show("不能自己嵌入自己！", "Xzy.EmbeddedApp");
                    return;
                }
                if (!value.ToLower().EndsWith(".exe"))
                {
                    MessageBox.Show("要嵌入的文件扩展名不是exe！", "Xzy.EmbeddedApp");
                }
                if (!File.Exists(value))
                {
                    MessageBox.Show("要嵌入的程序不存在！", "Xzy.EmbeddedApp");
                    return;
                }
                m_AppFilename = value;
            }
